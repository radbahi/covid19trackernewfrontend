...
export default class TestMap extends React.PureComponent {
    ...
    state = {tool: TOOL_NONE, value: INITIAL_VALUE, geographies: [], infectedAreas: []}
    Viewer = null
      ...
      <g className="provinces">
        {
          this.state.geographies.map((d,i) => (
            <path
              ...
              className="province"
              fill={ `rgba(38,50,56,${ 1 / this.state.geographies.length * i})` } // maybe this to change opacity?
              stroke="#FFFFFF"
              strokeWidth={ 0.5 }
      ...
      <g className="markers">
        {
          this.state.infectedAreas.map((city, i) =>{ 
            ...
            if (parseInt(this.props.user.location) === parseInt(city.id)) {
              return (             
                <circle
                  ...
                  r = { city.confirmed > 10 ? 10 + (city.confirmed * 0.01) : city.confirmed }
                  fill="#E91E63"
                  fill-opacity="0.7"
                  stroke="#FFFFFF"
                  className="marker"
           ...
           else if ( !this.props.user.location || this.props.user.status === 500) {
            return (
            <circle
              key={ `marker-${i}` }
              cx={ projection(coordinates)[0] }
